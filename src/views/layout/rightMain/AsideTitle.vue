    ...
    <div style="position: relative">
      ...
      <div class="titleNav" @click.right.prevent="mouseRight">
        <el-tag
          ...
          :class="{
            title: true,
            active: active === item.activeIndex ? true : false,
            marginR: index != getAsideTitle.length - 1
          }"
...
export default {
  ...
  data() {
    return {
      ...
      active: this.$store.getters.newactive,
  ...
  created() {
    ...
    if (loadBeforeRes != null) {
      if (
        ...
        this.$store.commit("newherder/delOrAddAside", loadBeforeRes.titleItem);
        ...
        this.$store.commit(
          ...
          loadBeforeRes.activeIndex
        );
        //设置增值变量为缓存的最大值，防止增值变量从0开始自增
        this.$store.commit("newherder/setCount", maxResult.activeIndex);
        this.$store.commit("newherder/setCrumbs", loadBeforeRes.crumbs);
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    toUrl(path, index) {
      //跳转路由
      this.$store.commit("newherder/setActiveIndex", index);
      this.$router.push({
        path: path
      });
    ...
    move(direction) {
      if (direction === "left") {
        let num = 0;
        num += (this.navWidth - this.pageWidth + 180) / 3;
